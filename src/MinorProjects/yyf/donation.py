...
master_table = pd.read_excel(master_table_name, convert_float=False, sheet_name=None, skiprows=0, parse_dates=True, na_values='').popitem()[-1].applymap(lambda x: x.replace('  ', ' ') if type(x) == str else x).dropna(subset=['备案编号'])
...
master_table['目标金额（元）'].fillna('', inplace=True)
